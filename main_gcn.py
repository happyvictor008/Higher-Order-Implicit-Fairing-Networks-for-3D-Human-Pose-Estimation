...
from models.h_gcn_ii import HGCNII
...
def main(args):
    ...
    #model_pos = GraphUNet(adj, edges_index,args.hid_dim, p_dropout=p_dropout, nodes_group=dataset.skeleton().joints_group() if args.non_local else None).to(device)
    #model_pos = Net(hidden_channels=2048, num_layers=9, alpha=0.5, theta=1.0,shared_weights=False, dropout=0.2,edges_index=edges_index).to(device)
    #model_pos = GCNII(nfeat=2,nlayers=64,nhidden=64,nclass=3,dropout=0.6,lamda = 0.5,  alpha=0.1, variant=False).to(device)
